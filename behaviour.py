...
def main(argv=None):
	...
	X_test = np.load(FLAGS.adversary_path_xo)[:100]
	X_test_adv = np.load(FLAGS.adversary_path_x)[:100]
	Y_test = np.load(FLAGS.adversary_path_y)[:100]
	...
	if FLAGS.is_autoencoder == 3:
		...
		orig_points = np.argmax(model.predict(X_test.astype('float32')))
		points = np.argmax(model.predict(X_test_adv.astype('float32')))
