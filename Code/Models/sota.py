...
def conv_stack(filters, side, activation, model, input_shape=None):
	if not input_shape:
		model.add(Convolution2D(filters, side, side, border_mode='same', W_regularizer=l2(0.0005), init='he_normal'))
	else:
		model.add(Convolution2D(filters, side, side, border_mode='same', input_shape=input_shape, W_regularizer=l2(0.0005), init='he_normal'))
...
def cnn_cifar100(learning_rate):
	...
	model.add(BatchNormalization())
	model.add(Dropout(0.5))
	model.add(Dense(300, W_regularizer=l2(0.0005), init='he_normal'))
	model.add(ELU())
	model.add(BatchNormalization())
	model.add(Dense(100, W_regularizer=l2(0.0005), init='he_normal'))
...
def allcnn_cifar100(learning_rate):
	...
	model = Sequential()
	model.add(Convolution2D(96, 3, 3, border_mode='same', input_shape=(3,32,32), W_regularizer=l2(0.005=1), init='he_normal'))
