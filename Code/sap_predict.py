...
from keras.layers import Conv2D, Dense, Input, add, Activation, GlobalAveragePooling2D, MaxPooling2D, Flatten
...
parser.add_argument('-dx','--data_x', type=str, default="", metavar='STRING',
				help='path to images to run inference on')
parser.add_argument('-dy','--data_y', type=str, default="", metavar='STRING',
...
if __name__ == "__main__":
	...
	for j in tqdm(range(X.shape[0])):
		...
		if j % 100 == 0:
			print("Accuracy so far for %d data : %f" % (j, acc / (j+1)))
	acc /= len(Y)
