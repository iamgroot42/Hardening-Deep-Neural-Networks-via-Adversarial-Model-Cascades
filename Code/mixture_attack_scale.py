...
def main(argv=None):
	...
	for model in models:
		...
		if model[0] == "cnn1x":
			...
			new_x = scale_images.scaleImages(new_x, 2)
			x = tf.placeholder(tf.float32, shape=(None, 3, 64, 64))
		elif model[0] == "cnn4x":
			new_x = scale_images.scaleImages(new_x, 4)
			x = tf.placeholder(tf.float32, shape=(None, 3, 128, 128))
		predictions = model[1](x)
		adv_x = helpers.fgsm(x, predictions, eps=FLAGS.fgsm_eps)
		print "Shape", x
		X_test_adv, = batch_eval(sess, [x], [adv_x], [new_x])
		...
		x_save = scale_down.scaleImages(X_test_adv, 32, 32)
		print" Saving shape", x_save.shape
		perturbed_data.append([x_save, Y_test_pm])
